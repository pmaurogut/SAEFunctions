...
make_G<-function(reStruct,coefficients_random,sigma,new_data,clean=TRUE){
...
#	Random effects matrices
	...
	G<-lapply(G,function(x,y)cor2var(x,y),y=sigma)
...
#	Variance of residuals
		weightsmodel<-rep(sigma,dim(new_data)[1])
		
	}else{

#	Variance of residuals

		weightsmodel<-Initialize(varStruct,new_data)
		weightsmodel<-1/varWeights(weightsmodel)
		weightsmodel<-weightsmodel*sigma
...
#	Covariances of residuals
	if(is.null(corStruct)){
		...
		sparseR<-Diagonal(x=weightsmodel)
		listR<-NULL
		...
		if(is.null(getGroupsFormula(corformula))){
			
			new_data$cor<-getCovariate(new_data,form=corformula)
			cors<-corMatrix(corStruct,new_data$cor)
			sparseR<-addstdDev(cors,weightsmodel)
			sparseR<-cor2var(sparseR)
			sparseR<-Diagonal(x=sparseR)
			index<-c(1:length(new_data[,1]))
			index_pos<-index
			...
			cors<-corMatrix(corStruct,covariates)
			#	Indexes
			index<-split(c(1:length(new_data[,1])),grps,drop=TRUE)
			index<-lapply(index,function(x)Diagonal(x=x))
			index<-bdiag(index)
			index<-diag(index)
			index_pos<-order(index)
	...
	list(R=sparseR,listR=listR,cor=cors,
...
makeCovMatrices<-function(model,new_data,clean=TRUE){
	
	if(exists("reStruct",model$modelStruct)){
		
		G<-make_G(model$modelStruct$reStruct,model$coefficients$random,new_data)
	...
	R<-make_R(model$modelStruct$corStruct,model$modelStruct$varStruct,model$sigma,new_data)
	res<-c(G,R)
	res
